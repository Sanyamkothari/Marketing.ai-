...
* The engine side is ready. Phase 3b added the `uplift` problem type, and
  ...
  checked by resolving it with the engine's own config loader from a copy of `library/configs/`,
...
So the use case stays **planned** in `ad_tech.yaml` and out of `library/configs/use_cases/`, and
...
`library/configs/use_cases/criteo_uplift.yaml`, flip `ad_tech.yaml` to `available`, update
