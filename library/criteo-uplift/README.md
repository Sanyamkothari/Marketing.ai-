...
resolving it from a copy of `configs/`; on a synthetic frame with the published columns the
uplift feature selection keeps exactly `f0` … `f11`). It is still **not installed** in
`configs/use_cases/`, and `ad_tech.yaml` still lists it as **planned**, because of reason
two.
...
2. Replace the invented `examples` in `use_case.yaml` with values from `data/prepared.csv`, copy it
   to `configs/use_cases/criteo_uplift.yaml`, flip `ad_tech.yaml`'s entry to `available`,
