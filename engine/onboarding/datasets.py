...
from engine.storage import Storage, StorageError, default_storage, upload_key
...
def run_source_key(record: RunRecord, file_format: str) -> str:
    """Key of the rows `record` read: its upload's source file, or the frame of its built dataset.

    A run reads exactly one of the two (`RunRecord.upload_id` or `dataset_id`), so anything that goes
    back to a run's rows after the pipeline - the generative modules do - asks here rather than
    assuming an upload. `file_format` is the profile's; a dataset frame is always parquet.
    """
    if record.dataset_id is not None:
        return dataset_key(record.dataset_id, DATASET_FRAME_FILENAME)
    if record.upload_id is None:
        raise ValueError(f"run {record.run_id} names neither an upload nor a dataset")
    return upload_key(record.upload_id, f"source.{file_format}")
