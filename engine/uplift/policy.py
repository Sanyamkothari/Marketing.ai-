...
**How many.** Candidates are ranked by predicted uplift, highest first (a stable sort, so ties keep
input order and the answer is deterministic). `N` is the longest prefix of that ranking that
...
number the page leads with is `N × observed uplift in the top N/rows share of the hold-out`, with the
bootstrap interval of that observed uplift scaled by `N`. The flow supplies it as a callable built on
`engine.uplift.metrics.bootstrap_uplift_at`; with no callable, or when the hold-out cannot measure
that share, the field is null and the page shows "—". Choosing nobody is exactly zero incremental
conversions, so `N = 0` reports `0` with a zero-width interval rather than asking the hold-out.
...
def choose_contacts(
    ...
    *,
    ...
    `eligible` defaults to every row.
    """
    import numpy as np
    ...
    candidates = np.flatnonzero((labels == Segment.PERSUADABLE.value) & allowed)
    if len(candidates) == 0:
        return selected, PolicyStopReason.NO_PERSUADABLES

    ranked = candidates[np.argsort(-lift[candidates], kind="mergesort")]
...
def recommend_policy(
    ...
    *,
    ...
    `fraction` of it, with its interval, or `None` when it cannot be measured; see the module
    docstring for how it becomes the expected incremental conversions.
    """
    started = time.perf_counter()
    lift = _finite_vector(uplift, "uplift")
    labels = _segment_vector(segments)
    allowed = _eligible_vector(eligible, len(lift))
    selected, reason = choose_contacts(lift, labels, policy, eligible=allowed)

    rows = len(lift)
    contacts = int(selected.sum())
    candidates = int(((labels == Segment.PERSUADABLE.value) & allowed).sum())
    expected = _expected_conversions(contacts, rows, observed_top_share)
    ...
    _LOGGER.info(
        "uplift_policy eligible_persuadables=%d contacts_recommended=%d stop_reason=%s",
        candidates,
        contacts,
        reason.value,
    )
...
def _expected_conversions(
    ...
    """`contacts × observed uplift in the top contacts/rows share`, interval scaled the same way."""
    if observed_top_share is None:
        return None
    if contacts == 0:
        return ConfidenceValue(value=0.0, ci_low=0.0, ci_high=0.0)
    observed = observed_top_share(contacts / rows)
    if observed is None:
        return None
