...
`api.main.create_app` mounts both routers since the integration into `main` (its PHASE-2 block), but
these tests keep to the smallest app that serves them, so a failure here is these routes' own:
`ConfigRootDep`/`StorageDep` read `app.state.config_root`/`data_dir` exactly as
`api.main.create_app` sets them, and the one `ConfigError` a bare app would otherwise let through
unhandled (`read_standard_schema` on a planned or unknown use case) is caught by importing
...
def test_standard_schema_of_a_planned_use_case_is_404(
    client_app: FastAPI, client: TestClient, tmp_path: Path
) -> None:
    # Nothing in the shipped configuration is planned any more (see tests/fixtures/planned.py).
    client_app.state.config_root = planned_config_root(tmp_path)
