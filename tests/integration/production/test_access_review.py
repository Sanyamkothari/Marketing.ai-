...
def test_anonymous_audit_events_are_rate_limited_per_address(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    # The limiter's window is the wall-clock minute (DEC-724). The loop below is held inside one
    # minute, so a slow machine that crosses a minute boundary mid-loop cannot reset the count and
    # turn this into a test of the clock (DEC-801).
    monkeypatch.setattr("api.access.time", SimpleNamespace(time=lambda: 1_790_000_010.0))
    app = local_app(tmp_path)
    client = TestClient(app)
