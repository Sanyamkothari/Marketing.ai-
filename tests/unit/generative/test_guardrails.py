...
from engine.llm import FakeLLMClient, FakeMode
...
def meter(mode: FakeMode = FakeMode.GROUNDED) -> Meter:
...
def test_a_judge_below_its_threshold_blocks_the_text() -> None:
    result = Guardrails(load_policy(), meter=meter(FakeMode.FAILING_JUDGE)).check(
        CLEAN, context(judges=("toxicity",), judge_values={})
    )
    assert result.outcome is GuardrailOutcome.BLOCKED
...
def test_a_judge_that_does_not_answer_in_json_scores_zero() -> None:
    ...
    result = Guardrails(load_policy(), meter=meter(FakeMode.MALFORMED)).check(
        CLEAN, context(judges=("toxicity",), judge_values={})
    )
