...
| `tests/integration/test_acceptance.py` | test_the_results_blocks_name_the_data_and_the_model | `assert f"key {PRIMARY_KEY}" in data_meta and f"target {TARGET}" in data_meta` | `assert f"ID {PRIMARY_KEY}" in data_meta and f"predicting {TARGET}" in data_meta` | The Data block's line reads "ID customer_id · predicting churned" instead of the jargon "key … · target …" (plan WP2, Results). The same two values are still asserted. |

Otherwise unchanged and still passing: `tests/integration/test_acceptance.py` (".summary" still carries "Training complete",
