...
**Re-filed 2026-09-23 (Plan D M58, DEC-885).** Still open. Plan D M58 asked for these plans and for Plan B, the Phase 5 design and Plans E and F. Only Plan D's own text was available, and it is the only one added. `docs/plans/README.md` lists the rest as missing rather than reconstructing them, and Phase 4b's plan stays at `docs/PHASE4B_PLAN.md`. Needed from: the plan owner (Minfy AI/ML team), the original files for Phase 2, 3a and 4a, Plan B, the Phase 5 design and Plans E and F, to be added unchanged.
...
**Re-filed 2026-09-23 (Plan D M58)** to the trunk. Still open, and outside Plan D's scope. Making `create_run_endpoint`'s dataset branch call `engine.scheduling.firing.start_dataset_run` is a refactor of a route that both Phase 4b's scheduling and Plan A's dataset runs depend on. Plan D only passed `requested_by` through both paths (DEC-862). Needed from: the trunk's owner of `api/routes/runs.py`, the refactor, with `tests/integration/test_runs_from_dataset.py` and the scheduling tests as its guard.
...
**Resolved 2026-09-23 (Plan D, DEC-800).** An announcement, not a request. The integration of Plan A, Phase 3b and the Phase 4b foundation merged every change listed above (DEC-800), and the fast suite and `make lint` pass over them on `main`. Nothing was asked of any branch. No owner action remains other than an optional review.
...
**Resolved 2026-09-23 (Plan D, DEC-859).** `POST /runs` refuses a **training** run whose resolved problem type is uplift, by override or by the use case's own configuration, with a synchronous `422 UPLIFT_REQUIRES_UPLIFT_ROUTE` that points to `POST /uplift/runs`, before a run exists. Scoring an uplift model stays on `POST /runs`. Plan D M53, in `api/routes/runs.py`, with a denied-role test.
...
**Resolved 2026-09-23 (Plan D, DEC-858).** `read_artefact` in `api/routes/runs.py` now also accepts `name in UPLIFT_ARTEFACTS`. It is still a whitelist, never a path join. `GET /runs/{run_id}/uplift/{name}` stays as an alias. Plan D M53.
...
**Resolved 2026-09-23 (Plan D, DEC-853, DEC-854, DEC-855).** Uplift training, scoring and campaign results are keyed by both columns. Treatment is per customer, and a customer in both arms of one campaign is refused (`TREATMENT_VARIES_WITHIN_ENTITY`). The hold-out is grouped by customer. `COMPOSITE_KEY_NOT_SUPPORTED` is gone from the uplift flow. Remaining limit, recorded in `docs/UPLIFT.md` §15: `POST /uplift/runs` takes an upload, not a `dataset_id`, and Phase 1's `POST /runs` takes a composite key only with a dataset, so multi-snapshot scoring files go through a dataset. Plan D M53; `tests/integration/uplift/test_uplift_two_column_keys.py`.
...
**Resolved 2026-09-23 (Plan D, DEC-657), as recorded.** Nothing was asked: the entry records an interface, and the hooks it names exist on `main` as described (DEC-657). Phase 5 reads it here. No owner action remains.
...
**Resolved 2026-09-23 (Plan D, DEC-854).** `docs/DATA_CONTRACT.md` §11 lists every uplift code with its severity, whether it can be acknowledged, its message and its suggestion: the six above plus M53's `TREATMENT_VARIES_WITHIN_ENTITY`. `tests/unit/uplift/test_uplift_two_column_keys.py` fails if `docs/DATA_CONTRACT.md` or `docs/UPLIFT.md` §3 misses a code of `UPLIFT_VALIDATION_CODES`. Plan D M53.
...
**Resolved 2026-09-23 (Plan D, DEC-858)**, differently from the entry's suggestion. Rather than sending the user to the uplift module's pages, Phase 1's pages (`ui/pages.js`, `ui/app.js`) now render the uplift artefacts themselves: treatment and control with the randomness check on Data, the Qini curve and AUUC on Model, and the segments and treat list on Output. They no longer request `prepare.json`, `drift.json` or `decile_lift.json` for an uplift run. The uplift module's own links are unchanged. Plan D M53; `tests/unit/uplift/test_phase1_pages_uplift.py`.
...
**Resolved 2026-09-23 (Plan D, DEC-887).** The module's apps get `tests/unit/production/scheduling_support.RecordingJobs`, which records a submitted training job without starting it. The assertions read `run.json` and the refusals, as before, so no training job outlives the suite.
...
**Resolved 2026-09-23 (Plan D M58).** The four fixes and their regression tests are on `main`, the integrations merged them, and the generative suites pass. Phase 3a has no branch left to review from. No owner action remains other than an optional review. The one judgement call a reviewer may still want to read is (3), refusing a document name clash.
...
**Resolved 2026-09-23 (Plan D, DEC-801).** Review only, and the edit was merged with Phase 4b Part 1 (DEC-801). The chain test passes on `main`, and Plan D's `0005_plan_d` extends the same chain (DEC-865). No owner action remains other than an optional review.
...
**Resolved 2026-09-23 (Plan D, DEC-801).** Review only, and the edit was merged with Phase 4b Part 1 (DEC-801). Plan D added its two tables to the same test (DEC-862, DEC-863), and CI runs it against a real PostgreSQL (DEC-879). No owner action remains other than an optional review.
...
**Resolved 2026-09-23 (Plan D, DEC-801).** Review only, and the edit was merged with Phase 4b Part 1 (DEC-801). The test passes on `main`. No owner action remains other than an optional review.
...
**Resolved 2026-09-23 (Plan D, DEC-801).** Review only, and the section was merged with Phase 4b Part 1 (DEC-801). `test_docs_honesty.py` passes over it. No owner action remains other than an optional review. The section's notification timelines are still placeholders pending Minfy's legal review, as the section itself says. That review belongs to the legal team, not to a branch.
...
**Resolved 2026-09-23 (Plan D, DEC-795).** Review only, and the edit was merged with Phase 4b Part 1 (DEC-795, DEC-801). The schema tests pass on `main`. No owner action remains other than an optional review.
...
**Resolved 2026-09-23 (Plan D, DEC-865, DEC-888).** `alembic/env.py` imports every platform row class and then admits `METADATA_TABLES | PLATFORM_TABLES` in `include_name` (`INCLUDED_TABLES`), in that order. `tests/unit/production/test_plan_d_migration.py` runs `alembic check` against a database migrated to head. DEC-865 supersedes DEC-797.
...
**Resolved 2026-09-23 (Plan D M58).** `docs/AWS_DEPLOYMENT.md` §11.1, the Phase 4b settings table's section, now opens with a one-line pointer to `docs/PRODUCTION.md`.
...
**Resolved 2026-09-23 (Plan D, DEC-880).** `POST /clients/{id}/onboarding-specs` and its replay call `api.routes.schedules.sync_recipe_retraining` after `save_spec`. It runs the same code as `POST /schedules/retraining/sync`, limited to the recipe's client and use case. It does nothing for a recipe without a label or with `scheduler_backend` `none` (DEC-782), and it is best-effort: a failure is logged by class name and the save still succeeds.
...
**Resolved 2026-09-23 (Plan D, DEC-862).** With sign-in on, the registry's `approved_by` and `promoted_by` are the signed-in username whatever the body says. With sign-in off they are stored as typed. The person who started the training run is refused with `403 SEPARATION_OF_DUTIES`, and every approval, rejection and promotion is recorded in `model_decision`. Plan D M54, in `api/routes/models.py`.
